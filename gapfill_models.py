...
def _gapfill_worker(args: Tuple[str, str, str, bool]) -> Tuple[str, Optional[str]]:
    """
    Worker function for parallel gapfilling.

    Args:
        args: Tuple of (model_path, output_dir, gapfilling_mode, atp_safe)

    Returns:
        Tuple of (model_name, output_path or None if failed)
    """
    model_path, output_dir, gapfilling_mode, atp_safe = args
    model_name = os.path.splitext(os.path.basename(model_path))[0]
...
def gapfill_all_models(
    ...
    worker_args = [
        (model_path, output_dir, gapfilling_mode, atp_safe)
        for model_path in all_models
    ]
