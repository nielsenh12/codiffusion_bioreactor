...
def _(display, nan, read_csv, taxonomy_linkage):
    ...
    def _create_heatmap(df, level, abundance_range, taxonomies, cmap=None, log=False):
        ...
        _clusterMap.ax_heatmap.set_xticklabels(_clusterMap.ax_heatmap.get_xticklabels(), fontsize=24)
...
def _(
    ...
    shannon_indices = {_day: shannon_index(list(_abundances.values())) for _day, _abundances in abundances_js.items() if _day in _sample_days}
...
def _(DataFrame, Series, display, load, read_csv, set_option, sns):
    ...
    df_4.rename(columns={'all samples $H_2$ feed': '$H_2$ loading $\\left(\\frac{mol}{min}\\right)$', 'all samples $H_2$ BT': '$H_2$ breakthrough $\\left(\\frac{mol}{min}\\right)$'}, inplace=True)
    pval_matrix.rename(columns={'all samples $H_2$ feed': '$H_2$ loading', 'all samples $H_2$ BT': '$H_2$ breakthrough'}, inplace=True)
    reduced = True
    if reduced:
...
def _(
    ...
    _days = {'I34': 149, 'J12': 161, 'K12': 182, 'L12': 189, 'M12': 210, 'N12': 238, 'P12': 283, 'Q': 300}
