...
class LocalGenomeConverter:
    ...
    def visualize_taxonomies(
        ...
        # for genome_path in glob(f"output_dir/*.json"):
        ...
        all_sub_elements = sorted({item for c in tax_info["counts"].values() for item in c})
        
        # Prepare stacked bar data
        group_names = list(tax_info["counts"].keys())
        group_values = np.zeros((len(all_sub_elements), len(group_names)))
        
        for j, group in enumerate(group_names):
            total = sum(tax_info["counts"][group].values())
            for i, sub in enumerate(all_sub_elements):
                group_values[i, j] = tax_info["counts"][group].get(sub, 0) #/total
        ...
        for i, sub in enumerate(all_sub_elements):
            ...
            for j, bar in enumerate(bars):
                val = int(tax_info["counts"][group_names[j]].get(sub, 0))  # correct lookup
